...
private:
	...
	virtual void Update(float frameTime) override
	{
		...
		{
			CPolygonPtr c1 = collision.polyA;
			CPolygonPtr c2 = collision.polyB;

			float bounciness = 1.f;

			float impulse = -(bounciness + 1.f) * (c1->speed - c2->speed | collision.normal);
			impulse /= ((1 / c1->density) + (1 / c2->density));

			Vec2 offset = collision.normal * (collision.distance - RADIUS * 2.f) * 0.5f;

			c1->position += offset;
			c2->position -= offset;

			c1->speed += (collision.normal * impulse) / c1->density;
			c2->speed -= (collision.normal * impulse) / c2->density;
		});
		...
		/*for (size_t i = 0; i < m_circles.size(); ++i)
		{
			for (size_t j = i + 1; j < m_circles.size(); ++j)
			{
				...
				if (diffPos.GetSqrLength() < 4.0f * RADIUS * RADIUS && ((diffSpeed | diffPos) < 0.0f))
				{
					...
					float impulse = -(bounciness + 1.f) * (c1->speed - c2->speed | normal);
					impulse /= ((1 / c1->density) + (1 / c2->density));

					Vec2 offset = normal * (diffPos.GetLength() - RADIUS * 2.f) * 0.5f;

					c1->position += offset;
					c2->position -= offset;

					c1->speed += (normal * impulse) / c1->density;
					c2->speed -= (normal * impulse) / c2->density;
				}
			}
		}*/
